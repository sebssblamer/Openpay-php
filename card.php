...
<script type="text/javascript">
        $(document).ready(function() {
            ...
            OpenPay.setId('m38tzl06ati97otvqcu0');
            OpenPay.setApiKey('pk_2fc784251d08402c80e2aa3fb5cf4adb');
            OpenPay.setSandboxMode(true);
            //Se genera el id de dispositivo
            var deviceSessionId = OpenPay.deviceData.setup("payment-form", "deviceIdHiddenFieldName");
            document.getElementById("device_id").value = deviceSessionId;
            $('#pay-button').on('click', function(event) {
                event.preventDefault();
                $("#pay-button").prop( "disabled", true);
                document.getElementById("token_id").value = OpenPay.token.extractFormAndCreate('payment-form', sucess_callbak, error_callbak);
            });

            var sucess_callbak = function(response) {
                var token_id = response.data.id;
                console.log(token_id + "cliente creado")
                $('#token_id').val(token_id);
                var form = document.getElementById("payment-form").submit();
                $.ajax({
...
<body>
    <div class="bkng-tb-cntnt">
        <div class="pymnts">
            <form action="#" method="post" id="payment-form" name="payment-form" action="cargo.php">
